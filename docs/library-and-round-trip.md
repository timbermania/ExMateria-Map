# The library, the CLI legs and the round-trip instrument

The reference material behind the add-on: what `dump` and `build` are, what
`build` guarantees, and the measurement that makes "byte-exact" a number
rather than a claim. The [README](../README.md) is the quick start; this is
everything under it.
...
Blender 5.2's system Python (3.14). A Blender add-on cannot `pip install`, so
the no-dependency rule is a constraint, not a preference.
...
### Or don't run either — the add-on vendors this package

An artist installing the Blender add-on never meets those two commands. The
add-on ships a verbatim copy of this package under `_vendor/`, so a map goes
from the extracted disc tree into a scene and back out as a patcher bundle
with no CLI trip: **File ▸ Import ▸ FFT Map (.GNS)** and **File ▸ Export ▸ FFT
Map bundle**. The bundle is byte-identical to what `exmateria-map-build`
writes. Patching the ISO is still `fft-iso-patcher`'s job. ADR-0004
decision 31.
